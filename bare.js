...
function siteHeaders(h) {
  ...
  if (h.includes('reddit.com') || h.includes('redd.it'))
    return { 'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin' };
  if (h.includes('instagram.com') || h.includes('twitter.com') || h.includes('x.com'))
...
function smartAccept(path) {
  const p = (path || '').toLowerCase();
  if (/\.(js|mjs|cjs)$/.test(p))                               return '*/*';
  if (p.endsWith('.css'))                                       return 'text/css,*/*;q=0.1';
  if (p.endsWith('.json'))                                      return 'application/json,*/*;q=0.9';
  if (/\.(png|jpg|jpeg|gif|webp|avif|svg|ico|bmp)$/.test(p))  return 'image/avif,image/webp,image/apng,image/*,*/*;q=0.8';
  if (/\.(mp4|webm|mkv|m3u8|ts|m4s)$/.test(p))                return '*/*';
  if (/\.(woff2?|ttf|otf|eot)$/.test(p))                      return '*/*';
...
exports.handler = async event => {
  ...
  if (!rawUrl) return {
    ...
    body: JSON.stringify({ versions: ['v1', 'v2'], language: 'NodeJS', project: { name: 'Celestial', version: '15.0.0' } }),
  };
  ...
  const reqH = {
    ...
    'accept':                     smartAccept(target.pathname),
  ...
  const doReq = () => new Promise((res, rej) => {
    ...
    const req = (isHttps ? https : http).request(
      { method, hostname: target.hostname, port, path: target.pathname + target.search, headers: reqH, rejectUnauthorized: false, timeout: 30000 },
      r => {
  ...
  for (const [k, v] of Object.entries(resp.headers)) {
    const kl = k.toLowerCase();
    if (!BLOCK_RECV.has(kl) && kl !== 'content-encoding' && kl !== 'content-length') {
